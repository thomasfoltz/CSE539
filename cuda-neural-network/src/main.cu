...
void loadModel(NeuralNetwork nn, const char* filePath) {
    ...
    for (NNLayer* layer : layers) {
        ...
        LinearLayer* linearLayer = dynamic_cast<LinearLayer*>(layer);
        if (linearLayer) {
			std::cout << "Layer is a LinearLayer" << std::endl;
            int weightsRows = linearLayer->getXDim();
            int weightsCols = linearLayer->getYDim();
            std::cout << "Weights Rows: " << weightsRows << ", Weights Cols: " << weightsCols << std::endl;

            Matrix weights(weightsRows, weightsCols);
			weights.allocateMemory();
            ...
            std::cout << "Layer is not a LinearLayer" << std::endl;
        }
    }
}
